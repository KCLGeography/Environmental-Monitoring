...
#include <Adafruit_ADS1015.h>     //ADS1x15 library. Available from https://github.com/adafruit/Adafruit_ADS1X15

//SD CARD HOUSEKEEPING
const int CS = 10;            //Chip Select pin for SD card
SdFat sd;
SdFile file;
char newfile[] = "NOx.csv";     //Name of file to be written to microSD
...
//ADC HOUSEKEEPING. Setup below for Alphasense 3-way AFE 810-0019-01 (Sensor 1 = NO2; Sensor 2 = O3; Sensor 3 = NO)
Adafruit_ADS1115 ads1(0x48);    //ADC for NO2 and O3 sensors. Default ADS1115 address (0X48)
Adafruit_ADS1115 ads2(0x49);    //ADC for NO sensors. Secondary ADS1115 address (0X49) set by connecting V with ADDR.
...
void loop() {
  ...
  int adsNO2_1, adsNO2_2, adsO3_1, adsO3_2, adsNO_1, adsNO_2; 
  adsNO2_1 = ads1.readADC_SingleEnded(0); //NO2 working electrode reading
  adsNO2_2 = ads1.readADC_SingleEnded(1); //NO2 aux electrode reading
  adsO3_1 = ads1.readADC_SingleEnded(2);  //O3 working electrode reading
  adsO3_2 = ads1.readADC_SingleEnded(3);  //O3 aux electrode reading
  adsNO_1 = ads2.readADC_SingleEnded(0);  //NO working electrode reading
  adsNO_2 = ads2.readADC_SingleEnded(1);  //NO aux electrode reading
  ...
  //prepare data to be written to SD card
      ...
      dataString += ",";
      dataString += adsNO2_1; //NO2 working electrode reading
       dataString += ",";
      dataString += adsNO2_2; //NO2 aux electrode reading
       dataString += ",";
      dataString += adsO3_1; //O3 working electrode reading
       dataString += ",";
      dataString += adsO3_2; //O3 aux electrode reading
       dataString += ",";
      dataString += adsNO_1; //NO working electrode reading
       dataString += ",";
      dataString += adsNO_2; //NO aux electrode reading
